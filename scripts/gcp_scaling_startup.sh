...
.venv/bin/python -u scripts/train_doserad_gpu.py --patients $PATS \
  --max-beamlets "$PER_PATIENT" --epochs 150 --device cuda --batch-size 24 \
  --model dota3d --d-model 256 --n-layers 8 --lr 3e-4 \
  --lr-schedule dota --restart-epochs 28 --weight-decay 0.05 --eval-subsample 96 --full-eval-every 25 \
  --gcs "$RUN" --out-dir /opt/bt/runs/dota10 2>&1 | tee /opt/bt/train.log
